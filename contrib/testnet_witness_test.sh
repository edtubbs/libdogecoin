...
SPV_TIMEOUT_SECONDS="${SPV_TIMEOUT_SECONDS:-120}"
...
echo "[3/5] Start spvnode in SMPV mode"
run_and_log "spvnode" timeout "$SPV_TIMEOUT_SECONDS" ./spvnode $TESTNET_FLAG -l -c -d -x -p -a "$SPV_ADDR" scan > "$TMPDIR/spvnode.log" 2>&1 &
SPV_PID=$!
sleep 8
