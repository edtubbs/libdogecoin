...
static void test_trezor_vectors(void)
{
    ...
    { const char* sh[] = { v1_s1 };
    slen = sizeof(secret); rc = dogecoin_slip0039_recover_secret(sh, 1, secret, &slen);
    ...
    slen = sizeof(secret); rc = dogecoin_slip0039_recover_secret(sh, 2, secret, &slen);
    ...
    slen = sizeof(secret); rc = dogecoin_slip0039_recover_secret(sh, 3, secret, &slen);
    u_assert_int_eq(rc, -1); /* expected failure */ }
    ...
    slen = sizeof(secret); rc = dogecoin_slip0039_recover_secret(sh, 5, secret, &slen);
