...
void test_bip39()
{
    ...
    if (entropy_out == NULL) {
        fprintf(stderr, "ERROR: Failed to allocate memory for entropy\n");
    }
    ...
    /* 
     * Additional Electrum v2 test vectors with passphrase and different versions
     * Reference: Electrum v2 uses standard PBKDF2-HMAC-SHA512 with:
     *   - password: mnemonic (normalized)
     *   - salt: "electrum" + passphrase
     *   - iterations: 2048
     *   - dkLen: 64 bytes
     * Seed version detection via HMAC-SHA512("Seed version", mnemonic):
     *   - First hex nibble + 2 = prefix length
     *   - Prefix parsed as hex = version
     * Test vectors generated by libdogecoin implementation
     */
    ...
    /* Expected: PBKDF2-HMAC-SHA512(password=mnemonic, salt="electrum" + "mypassphrase", rounds=2048) */
    ...
    /* Expected: PBKDF2-HMAC-SHA512(password=mnemonic, salt="electrum", rounds=2048) */
    ...
    /* 
     * Additional Electrum v1 test vectors with valid 12-word mnemonic
     * Reference: Electrum v1 uses 1626-word list (wordlist_electrum from include/bip39/electrum.h)
     * Seed derivation process (non-standard, different from BIP39):
     *   1. Decode 12-word mnemonic using base-1626 encoding to get 16-byte seed
     *   2. If decoding fails, use fallback: SHA256(mnemonic + (" " + passphrase if non-empty))
     *   3. Stretch the seed: stretched = seed, then repeat 100,000 times: stretched = SHA256(stretched + seed)
     *   4. Result is 32 bytes, padded to 64 bytes with zeros
     * Test vectors generated by libdogecoin's dogecoin_seed_from_electrum_v1_mnemonic()
     * Verified against Electrum wallet v1 behavior
     */
    const char* v1_mn2 = "like just love know never want time out there make look eye";
    const char* v1_pass2 = "";
    
    memset(seed, 0, sizeof(seed));
    u_assert_int_eq(dogecoin_seed_from_electrum_v1_mnemonic(v1_mn2, v1_pass2, seed), 0);
     ...
     * Test vector generated by libdogecoin implementation
     */
