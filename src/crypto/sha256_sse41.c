...
/*
 * libdogecoin SHA-256 multi-way transform
 * Ported from Bitcoin Core src/crypto/sha256_sse41.cpp
 * Original code is MIT-licensed; see Bitcoin Core COPYING.
 */
...
static inline __m128i K(uint32_t x) { return _mm_set1_epi32(x); }

static inline __m128i Add2(__m128i x, __m128i y) { return _mm_add_epi32(x, y); }
static inline __m128i Add3(__m128i x, __m128i y, __m128i z) { return Add2(Add2(x, y), z); }
static inline __m128i Add4(__m128i x, __m128i y, __m128i z, __m128i w) { return Add2(Add2(x, y), Add2(z, w)); }
static inline __m128i Add5(__m128i x, __m128i y, __m128i z, __m128i w, __m128i v) { return Add2(Add3(x, y, z), Add2(w, v)); }
static inline __m128i Inc2(__m128i* x, __m128i y){ *x = Add2(*x, y); return *x; }
static inline __m128i Inc3(__m128i* x, __m128i y, __m128i z){ *x = Add3(*x, y, z); return *x; }
static inline __m128i Inc4(__m128i* x, __m128i y, __m128i z, __m128i w){ *x = Add4(*x, y, z, w); return *x; }
static inline __m128i Xor2(__m128i x, __m128i y) { return _mm_xor_si128(x, y); }
static inline __m128i Xor3(__m128i x, __m128i y, __m128i z) { return Xor2(Xor2(x, y), z); }
static inline __m128i Or(__m128i x, __m128i y) { return _mm_or_si128(x, y); }
static inline __m128i And(__m128i x, __m128i y) { return _mm_and_si128(x, y); }
static inline __m128i ShR(__m128i x, int n) { return _mm_srli_epi32(x, n); }
static inline __m128i ShL(__m128i x, int n) { return _mm_slli_epi32(x, n); }

static inline __m128i Ch(__m128i x, __m128i y, __m128i z) { return Xor2(z, And(x, Xor2(y, z))); }
static inline __m128i Maj(__m128i x, __m128i y, __m128i z) { return Or(And(x, y), And(z, Or(x, y))); }
static inline __m128i Sigma0(__m128i x) { return Xor3(Or(ShR(x, 2), ShL(x, 30)), Or(ShR(x, 13), ShL(x, 19)), Or(ShR(x, 22), ShL(x, 10))); }
static inline __m128i Sigma1(__m128i x) { return Xor3(Or(ShR(x, 6), ShL(x, 26)), Or(ShR(x, 11), ShL(x, 21)), Or(ShR(x, 25), ShL(x, 7))); }
static inline __m128i sigma0(__m128i x) { return Xor3(Or(ShR(x, 7), ShL(x, 25)), Or(ShR(x, 18), ShL(x, 14)), ShR(x, 3)); }
static inline __m128i sigma1(__m128i x) { return Xor3(Or(ShR(x, 17), ShL(x, 15)), Or(ShR(x, 19), ShL(x, 13)), ShR(x, 10)); }

/** One round of SHA-256. */
static inline void __attribute__((always_inline)) Round(__m128i a, __m128i b, __m128i c, __m128i* d, __m128i e, __m128i f, __m128i g, __m128i* h, __m128i k)
{
    __m128i t1 = Add4(*h, Sigma1(e), Ch(e, f, g), k);
    __m128i t2 = Add2(Sigma0(a), Maj(a, b, c));
    *d = Add2(*d, t1);
    *h = Add2(t1, t2);
}

static inline __m128i Read4(const unsigned char* chunk, int offset) {
...
static inline void Write4(unsigned char* out, int offset, __m128i v) {
...
void Transform_4way(unsigned char* out, const unsigned char* in)
{
    ...
    Round(a, b, c, &d, e, f, g, &h, Add2(K(0x428a2f98ul), w0 = Read4(in, 0)));
    Round(h, a, b, &c, d, e, f, &g, Add2(K(0x71374491ul), w1 = Read4(in, 4)));
    Round(g, h, a, &b, c, d, e, &f, Add2(K(0xb5c0fbcful), w2 = Read4(in, 8)));
    Round(f, g, h, &a, b, c, d, &e, Add2(K(0xe9b5dba5ul), w3 = Read4(in, 12)));
    Round(e, f, g, &h, a, b, c, &d, Add2(K(0x3956c25bul), w4 = Read4(in, 16)));
    Round(d, e, f, &g, h, a, b, &c, Add2(K(0x59f111f1ul), w5 = Read4(in, 20)));
    Round(c, d, e, &f, g, h, a, &b, Add2(K(0x923f82a4ul), w6 = Read4(in, 24)));
    Round(b, c, d, &e, f, g, h, &a, Add2(K(0xab1c5ed5ul), w7 = Read4(in, 28)));
    Round(a, b, c, &d, e, f, g, &h, Add2(K(0xd807aa98ul), w8 = Read4(in, 32)));
    Round(h, a, b, &c, d, e, f, &g, Add2(K(0x12835b01ul), w9 = Read4(in, 36)));
    Round(g, h, a, &b, c, d, e, &f, Add2(K(0x243185beul), w10 = Read4(in, 40)));
    Round(f, g, h, &a, b, c, d, &e, Add2(K(0x550c7dc3ul), w11 = Read4(in, 44)));
    Round(e, f, g, &h, a, b, c, &d, Add2(K(0x72be5d74ul), w12 = Read4(in, 48)));
    Round(d, e, f, &g, h, a, b, &c, Add2(K(0x80deb1feul), w13 = Read4(in, 52)));
    Round(c, d, e, &f, g, h, a, &b, Add2(K(0x9bdc06a7ul), w14 = Read4(in, 56)));
    Round(b, c, d, &e, f, g, h, &a, Add2(K(0xc19bf174ul), w15 = Read4(in, 60)));
    ...
    // Output
    Write4(out, 0, Add2(a, K(0x6a09e667ul)));
    Write4(out, 4, Add2(b, K(0xbb67ae85ul)));
    Write4(out, 8, Add2(c, K(0x3c6ef372ul)));
    Write4(out, 12, Add2(d, K(0xa54ff53aul)));
    Write4(out, 16, Add2(e, K(0x510e527ful)));
    Write4(out, 20, Add2(f, K(0x9b05688cul)));
    Write4(out, 24, Add2(g, K(0x1f83d9abul)));
    Write4(out, 28, Add2(h, K(0x5be0cd19ul)));
}

/* Public C symbol used by sha2.c dispatcher. */
void libdogecoin_sha256d64_sse41(unsigned char* out, const unsigned char* in)
{
    Transform_4way(out, in);
}
